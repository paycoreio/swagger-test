...
class SwaggerRequester
{
    ...
    public static function setAuth($user, $password = null)
    {
        if(null === $password){
            $password = $user;
        }
        self::$auth = base64_encode($user . ':' . $password);
    }
    ...
    public function send()
    {
        ...
        if(null!==self::$auth){
            $this->requestHeader['Authorization'] = 'Basic ' . self::$auth;
        }
        ...
        if ($this->statusExpected !== $statusReturned) {
            throw new StatusCodeNotMatchedException("Status code not matched $statusReturned", $responseBody);
        }
        ...
        if (count($this->assertHeader) > 0) {
            foreach ($this->assertHeader as $key => $value) {
                if (!isset($responseHeader[$key]) || false === strpos($responseHeader[$key][0], $value)) {
